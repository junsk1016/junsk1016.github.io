---
layout: single
title: "이미지 색공간 변환 및 각 채널 visualization(RGB to BGR, YUV, HSV, Lab) - OpenCV(C++)"
categories:
  - OpenCV
classes: wide
tags:
...
RGB image를 입력받아서 다른 색공간 으로 변환 후 각 채널을 보는 것을 포스팅하겠습니다.
...
int main()
{
	Mat scenery, BGR_scenery, YUV_scenery, HSV_scenery, Lab_scenery; // 원본과 변환을 위한 Mat 객체 선언
  // 원본, RGB변환, YUV변환, HSV변환, Lab변환
	Mat BGR[3], YUV[3], HSV[3], Lab[3]; // split을 위한 Mat 객체 선언
	...
	if (num == 1) // BGR 선택
	{
		cvtColor(scenery, BGR_scenery, COLOR_RGB2BGR);
		split(BGR_scenery, BGR);
		imshow("BGR-B", BGR[0]);
		imshow("BGR-G", BGR[1]);
		imshow("BGR-R", BGR[2]);
	}
	else if (num == 2) // YUV 선택
	{
