...
int main()
{
	...
	if (num == 1)// webcam으로 입력/imshow/출력/녹화 버튼
	{
		...
		while (true) // 종료 시까지 영상 출력
		{
			...
			if (key == 27) // esc key 누르면 종료
				break;
			else if ((key == 82) || (key == 114)) // 녹화 버튼(r 또는 R)을 눌렀을 때
			{
				cout << "녹화가 시작됩니다.\n\n";
				...
				while (true)
				{
					...
					imshow("Live", frame);
					outputVideo.write(frame);// 이미지를 계속 저장되기 때문에 동영상으로 저장되게 하는 거임
	...
	else if (num == 2) // video file로 입력/imshow/출력/녹화 버튼
	{
		...
		while (cap.isOpened())
		{
			...
			if (waitKey(10) == 27) // esc key 누르면 종료로 설정
				// cout << "\n프로그램을 종료합니다!" << endl; <- 이 줄이 있으면 동영상 재생이 되지않고 바로 종료되는 에러 발생
				break;
			else if ((waitKey(10) == 82) || (waitKey(10) == 114)) // 녹화 버튼을 눌렀을 때
			{
				cout << "녹화가 시작됩니다.\n\n";
				...
				while (true)
				{
					...
					if (frame.empty())
					{
						cerr << "캡쳐 실패 or 동영상 재생이 완료되었습니다." << endl;
						break;
					}
	...
	{
		cout << "1 또는 2를 입력하세요. 프로그램이 종료됩니다.\n";
		return -1;
	}
